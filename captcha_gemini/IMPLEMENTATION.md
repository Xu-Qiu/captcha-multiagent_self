...
- 网格/顺序类题目要求：只返回行优先索引（外部 1 基/内部会换算），不要返回像素坐标；识别页展示的 PNG 会叠加实际行列的网格线辅助读数。执行侧会尝试将坐标换算为索引作为兜底，但推荐直接输出索引；顺序题缺少任意一个索引（长度≠k 或有重复）即视为错误。
- 点击类：必须由 LLM 根据坐标轴读出中心点，不能直接暴露存储答案；复审若给出修正 calc_center，则执行 Agent 以该修正点为准。
